...
#' @import data.table
#' @export
zip_dates <- function(dates, by = NULL, format = NULL, check_duplicate = FALSE) {
  # 1.1 guess parmater `by`
  if (is.null(by)) {
    ndays <- table(year(dates)) %>%
      median()
    # If no duplicated dates, ndays = 1, 12, 365/366 are corresponding to year, month, day
    # respectively.
    if (ndays <= 5) {
      by <- "year"
    } else if (ndays <= 30) {
  ...
  str_left <- info[, str_func(x, format), .(grp)]$V1 %>%
    paste(collapse = ", ")
  str_miss <- ""
  ...
  if (n_miss > 0) {
    ...
    str_miss <- info_miss[, str_func(x, format), .(grp)]$V1 %>%
      paste(collapse = ", ")
  }

  data.table(n_complete = n, n_left = n - n_miss, n_miss, perc_miss, date_begin, date_end, str_left, str_miss)
}
