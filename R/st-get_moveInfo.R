...
movePeriod <- function(st) {
  ...
  st_moveInfo <- plyr::ddply(st, .(site), function(d) {
    ...
    if (!is_second) {
      ...
      d[
        , .(period_date_begin = min(date), period_date_end = max(date), date_begin = date_begin0, date_end = date_end0),
      ...
      d[, .(
        period_date_begin = min(period_date_begin), period_date_end = max(period_date_end), date_begin = date_begin0,
        date_end = date_end0
      ), .(site, tag, lon, lat, alt)]
    }
  }, .progress = "text") %>%
...
moveTimes <- function(st_moveInfo) {
  suppressWarnings({
    st_moveInfo %<>%
      dplyr::mutate(n_all = difftime(date_end, date_begin) %>%
        as.numeric() %>%
        add(1), n_period = difftime(period_date_end, period_date_begin, units = "days") %>%
        as.numeric() %>%
        add(1))
    st_moveInfo[, `:=`(dist = distToCentralPeriod(lon, lat, n_period), moveTimes = max(tag)), .(site)]
  })
  st_moveInfo %<>%
    reorder_name(c("site", "moveTimes", "tag"))
  st_moveInfo[, 7:10] %<>%
    map(as.Date)
  st_moveInfo
}
