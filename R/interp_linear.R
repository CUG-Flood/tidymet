...
#'
#' @export
interp_linear <- function(xx, stationInfo,
    dist, smax = 100, nmax = 10,
    sinkfile = NULL,
    ...)
{
            ...
            for (j in seq_along(id)) {
                ...
                if (length(utime) == 0) {
                    ...
                    if (length(which(!is.na(y_trim) & is.na(x_trim))) == 0) {
                        ...
                        # lm_fit <- lm(xt ~ yt)
                        # ss <- summary.lm(lm_fit)$coefficients
                        lm_fit2 <- .lm.fit(cbind(1, yt), xt)
                        ss2 <- summary_lm(lm_fit2)

                        ## if lm coefficient significant
                        if (ss2[2, 4] > 0.05) {
                            # cat(sprintf('\t%02dth: failed!
                            # 站点%s线性拟合不能通过显著性检验!\n', j, station))
                        } else {
                            ypred2 <- cbind(1, y_trim[Ix_miss]) %*% lm_fit2$coefficients %>% as.numeric()
                            ...
                            interp_infoI[[j]] <-
                                data.frame(site = station, ubegin, uend, n_interp, n_missLeft, dist = dist.tmp[id][j])
                            # cat(sprintf('\t%02dth: success! 站点%s插值%s ~
                            # %s共%d个值，余留%d空值\n', j, station, begin(date_miss),
                            # end(date_miss), n_interp, n_missLeft))
                            if (n_missLeft == 0) {
                                # cat(sprintf('[%03d] ============== end ============\n', i))
