...
date_daily <- seq(as.Date("1951-01-01"), as.Date("2014-12-31"),
    by = "day")
ndays <- length(Id)
## ---------------------------------------------------------------------------
...
stations <- data.frame(station = paste0("v", seq_along(lat)),
    lat, lon, alt)
write.table(stations, file = "met2474_stations.txt", row.names = F,
    sep = "\t", quote = F)
...
for (i in 1:(nvars)) {
    ...
    if (i <= nvars - 3) {
        ...
        for (j in 1:nstd) xx[, j] <- as.numeric(x[, , , j])[Id]
        x_new[[i]] <- xx
    } else {
        x_new[[i]] <- x
    }
...
# ## detect missing information colnames(xx) <- paste0('v',
# 1:nstd) info <- MissInfo.matrix(xx, date_daily, showInfo =
# TRUE,infodays = TRUE, collapse = '\r\n', progress =
# FALSE) length(which(info$missingPerc < 10))/nstd*100
