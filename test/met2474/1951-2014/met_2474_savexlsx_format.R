...
# time <- info$dim$time$vals %>% as.Date(origin =
# '1970-01-01')
...
info.list <- lapply(1:4, function(i) lapply(stationInfo, `[[`, i)) %>% 
    set_names(c("origin", "naApprox", "linear", "hisavg"))
info <- lapply(info.list, function(x) do.call(cbind, x)[, -seq(9, 128, 8)])

x1 <- c("station", rep(colnames(info.list$origin$EVAP)[-1], 16))
x2 <- rep("", 113)
x2[seq(2, 113, 7)] <- varnames
...
for (i in 1:nrow(stations)) {
    ...
    # set_rownames(format(seq(ubegin, uend, by='day'),
    # '%Y-%m-%d'))
    df <- data.table(time = format(as.Date(ubegin:uend)), dmat,
        stringsAsFactors = F)
    ## 对信息进行压缩 info <- paste(format(begins,'%Y%m%d'),
    ## format(ends,'%Y%m%d'), sep = ':')
    fname <- sprintf("met2474/met2474_csv/%03dth_%s.csv", i, site[i])
    # fname <- paste0('data/OriginDataInput_xlsx/', site[i],
    # stations$Name[i], '.xlsx')
